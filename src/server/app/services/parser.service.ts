...
const handleParseCosmwasmExecuteContract = async (input: ParseInput) => {
  ...
  const res = await oraichainTatum.simulate.simulate(input.sender, msgs)

  const parserRes = await oraichainTatum.ammV2.parseSwapAndAction({sender: input.sender, events: res.data.result!.events, message: msgs})
